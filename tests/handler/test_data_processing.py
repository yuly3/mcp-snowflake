...
from mcp_snowflake.handler.data_processing.common import (
...
class TestProcessRowData:
    ...
    def test_process_row_data_success(self) -> None:
        ...
        result = process_row_data(raw_row)
...
class TestProcessMultipleRowsData:
    ...
    def test_process_multiple_rows_data_with_warnings(self) -> None:
        ...
        assert (
            result["processed_rows"][0]["complex_num"] == "<unsupported_type: complex>"
        )
        assert result["processed_rows"][1]["id"] == 2
        assert result["processed_rows"][1]["name"] == "Bob"
        assert (
            result["processed_rows"][1]["complex_num"] == "<unsupported_type: complex>"
        )

        # Warning should be deduplicated
        assert len(result["warnings"]) == 1
        assert (
            "Column 'complex_num' contains unsupported data type" in result["warnings"]
        )
...
class TestTypedDictStructures:
    ...
    def test_row_processing_result_structure(self) -> None:
        """Test RowProcessingResult structure."""
        result: RowProcessingResult = {
            "processed_row": {"id": 1, "name": "test"},
            "warnings": ["test warning"],
        }

        assert "processed_row" in result
        assert "warnings" in result
        assert isinstance(result["processed_row"], dict)
        assert isinstance(result["warnings"], list)

    def test_data_processing_result_structure(self) -> None:
        """Test DataProcessingResult structure."""
        result: DataProcessingResult = {
            "processed_rows": [{"id": 1}, {"id": 2}],
            "warnings": ["test warning"],
        }

        assert "processed_rows" in result
        assert "warnings" in result
        assert isinstance(result["processed_rows"], list)
        assert isinstance(result["warnings"], list)
...
class TestDataProcessingProperties:
    ...
    def test_process_row_data_with_json_compatible_types(
        ...
        # Warnings should only contain strings
    ...
    def test_process_multiple_rows_data_properties(
        ...
        # Each processed row should be a dict
        for processed_row in result["processed_rows"]:
